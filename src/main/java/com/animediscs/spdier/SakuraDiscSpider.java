...
public class SakuraDiscSpider {
    ...
    private int findRank(String input) {
        ...
        return matcher.find() ? HelpUtil.parseNumber(matcher.group(2)) : 0;
    }
    ...
    private int parsePrpt(String text) {
        ...
        if (matcher.find()) {
            return HelpUtil.parseNumber(matcher.group(2));
        } else {
            return 0;
        }
    }
    ...
    private void updateCapt(Document document, DiscSakura discSakura, Date release) {
        ...
        if (matcher.find()) {
            discSakura.setCapt(HelpUtil.parseNumber(matcher.group(1)));
            discSakura.setSday(HelpUtil.parseNumber(matcher.group(2)));
        } else {
