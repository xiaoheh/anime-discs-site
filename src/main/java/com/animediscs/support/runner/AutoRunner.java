...
import static com.animediscs.support.HelpUtil.nullSafeGet;
...
//        newTimerSchedule("Sakura碟片", 14, 1800, () -> addUpdateSakuraAllTask(900));
    ...
    private Supplier<Boolean> needUpdateSakura(Disc disc, int second) {
        return () -> isNeedUpdate(HelpUtil.nullSafeGet(disc.getSakura(), DiscSakura::getPadt), second);
    }

    private Supplier<Boolean> needUpdateAmazon(Disc disc, int second) {
        return () -> isNeedUpdate(HelpUtil.nullSafeGet(disc.getAmazon(), DiscAmazon::getPadt), second);
    }
